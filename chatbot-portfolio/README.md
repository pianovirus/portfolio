...
PostgreSQL · pgvector 하이브리드 검색 + Cross-encoder 리랭킹 · 검색 실패를 보정하는 자기교정(CRAG) 루프 · 멀티모달 문서 처리(PDF · PPTX · Notion)<br/>
...
![CRAG](https://img.shields.io/badge/Self--Correcting-CRAG%20Loop-f59e0b?style=for-the-badge)
...
- **🧠 LangGraph 자기교정 파이프라인** — 질문 분류 → 멀티턴 맥락 질의 재작성("그거/아까" 해소) → 검색 → 근거 기반 프롬프트. **검색 실패 시 조건을 완화해 재검색하는 Corrective RAG(CRAG) 루프**로 미응답 자동 복구
